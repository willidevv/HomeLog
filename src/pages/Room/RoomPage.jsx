...
export default function RoomView({ activePlan, activeRoom, filteredItems, maintenanceRecords, openModal, handleDelete, setActiveRoomId, setActiveItemId }) {
  return (
    <div className="animate-in fade-in slide-in-from-right-8 duration-300">
      {/* Header Section */}
      <div className="mb-8">
        <button onClick={() => setActiveRoomId(null)} className="flex items-center gap-2 text-navy-300 hover:text-gold-400 transition-colors mb-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-navy-300 text-lg">{activeRoom?.description || 'Kelola inventaris barang di ruangan ini.'}</p>
          </div>
          <button 
      ...
      {filteredItems.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredItems.map(item => {
            ...
            const statusBg = hasMaintenance ? 'bg-green-500/10 border-green-500/20' : 'bg-amber-500/10 border-amber-500/20';
            const statusText = hasMaintenance ? `Terakhir dirawat: ${lastMaintenance.maintenanceDate}` : 'Belum pernah dirawat';

            return (
              <div key={item.id} className="group relative bg-navy-900/40 backdrop-blur-sm border border-navy-800/50 rounded-2xl hover:border-gold-500/50 hover:shadow-xl hover:shadow-gold-500/10 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col">
                ...
                <div className="relative z-10 p-6 flex flex-col gap-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-cream-50 mb-2 group-hover:text-gold-300 transition-colors">{item.name}</h3>
                      {item.description && <p className="text-sm text-navy-300 line-clamp-2 mb-3">{item.description}</p>}
                  ...
                  <div className="pt-5 border-t border-navy-800/30 flex items-center justify-between">
                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                      <button onClick={() => openModal('item', item)} className="p-2 text-navy-400 hover:text-gold-400 hover:bg-gold-500/10 rounded-lg transition-all duration-200" title="Edit">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete('item', item.id)} className="p-2 text-navy-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-all duration-200" title="Hapus">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                    <button onClick={() => setActiveItemId(item.id)} className="text-gold-400 font-medium text-sm px-4 py-2 hover:bg-gold-500/10 rounded-lg transition-all duration-200 flex items-center gap-2 group-hover:gap-3">
