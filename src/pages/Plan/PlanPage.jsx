...
export default function PlanView({ activePlan, filteredRooms, items, openModal, handleDelete, setActivePlanId, setActiveRoomId }) {
  return (
    <div className="animate-in fade-in slide-in-from-right-8 duration-300">
      {/* Header Section */}
      <div className="mb-8">
        <button onClick={() => setActivePlanId(null)} className="flex items-center gap-2 text-navy-300 hover:text-gold-400 transition-colors mb-4">
      ...
      {filteredRooms.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredRooms.map(room => (
            <div key={room.id} className="group relative bg-navy-900/40 backdrop-blur-sm border border-navy-800/50 rounded-2xl hover:border-gold-500/50 hover:shadow-xl hover:shadow-gold-500/10 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col h-full" onClick={() => setActiveRoomId(room.id)}>
              ...
              <div className="relative z-10 p-6 flex-1 flex flex-col justify-center items-center text-center">
                ...
                <h3 className="text-xl font-bold text-cream-50 mb-2 group-hover:text-gold-300 transition-colors">{room.name}</h3>
                {room.description && <p className="text-sm text-navy-300 line-clamp-2 mb-4 px-2">{room.description}</p>}
              ...
              <div className="relative z-10 px-6 py-3 border-t border-navy-800/30 bg-navy-900/50 flex justify-between items-center" onClick={e => e.stopPropagation()}>
                <div className="flex gap-1">
                  <button onClick={() => openModal('room', room)} className="p-2 text-navy-400 hover:text-gold-400 hover:bg-gold-500/10 rounded-lg transition-all duration-200" title="Edit">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete('room', room.id)} className="p-2 text-navy-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-all duration-200" title="Hapus">
